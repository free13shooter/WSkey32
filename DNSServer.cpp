...
void DNSServer::processNextRequest()
{
  ...
  //Serial.printf("\n DNS:_currentPacketSize=%d", _currentPacketSize);
  ...
  if (_currentPacketSize)
  {
    ...
    {
      //Serial.printf("\n DNS:requestIncludesOnlyOneQuestion");
...
void DNSServer::replyWithIP()
{
  ...
  //---подмена---
  String hnam(getDomainNameWithoutWwwPrefix());
  #ifdef DEBUG_ESP_DNS
    Serial.printf("\nDNS: name=%s ",hnam.c_str());
  #endif
  IPAddress _hip=ntoh(hnam);
  if(_hip!=(uint32_t)0){
    _resolvedIP[0]=_hip[0];_resolvedIP[1]=_hip[1];_resolvedIP[2]=_hip[2];_resolvedIP[3]=_hip[3];
    #ifdef DEBUG_ESP_DNS
      Serial.printf("\nDNS: _resolvedIP changed to %s",IPAddress(_resolvedIP[0],_resolvedIP[1],_resolvedIP[2],_resolvedIP[3]).toString().c_str());
    #endif
  }
  //else {_resolvedIP[0]=apIP[0];_resolvedIP[1]=apIP[1];_resolvedIP[2]=apIP[2];_resolvedIP[3]=apIP[3];}
  
  _udp.write(_resolvedIP, sizeof(_resolvedIP)); // The IP address to return
  _udp.endPacket();

  #ifdef DEBUG_ESP_DNS
    DEBUG_OUTPUT.printf("\nreplyWithIP:DNS responds= %s for %s\n",
...
#ifdef DEBUG_ESP_DNS
   char *t=(char*)malloc(sizeof(DNSHeader)+1);
   memcpy(t,_buffer,sizeof(DNSHeader)+1);t[sizeof(DNSHeader)]=0;
   DEBUG_OUTPUT.printf("replyWithCustomCode: remoteIP()= %s, port=%d\n , buff=%s", IPAddress(_udp.remoteIP()).toString().c_str(), _udp.remotePort(), t );
   free(t);
#endif
}
