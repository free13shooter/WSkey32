...
bool ESP32_FTPClient::GetFTPAnswer (char* result, int offsetStart) {
  ...
  if(!client.available()){
    ...
    _isConnected = false;
    isConnected();
    //FTPdbgn("GetFTPAnswer: !client.available");
    Serial.print(F("\n->GetFTPAnswer: TIMEOUT !client.available<-"));
    return false;
  }
...
bool ESP32_FTPClient::WriteData (unsigned char * data, int dataLength) {
  FTPdbgn(F("Writing"));
...
bool ESP32_FTPClient::ContentList(const char * dir, String * list) {
  ...
  client.print(F("MLSD"));
  client.println(F(dir));
  if(!GetFTPAnswer(_resp))return false;
...
bool ESP32_FTPClient::ContentListWithListCommand(const char * dir, String * list) {
  char _resp[ sizeof(outBuf) ];
  uint16_t _b = 0;
